...
def create_task_notification(request):
    ...
    if request.method == 'POST':
        ...
        #send_email(mail, "You have a new Task.")
...
def WorkerRejectTask(request):
    if request.method == "PUT":
        ...
        try:
            ...
            # Check if the worker_decision is already 'Accept'
            if notification.worker_decision == 'Accept':
                return JsonResponse({"message": "You have already accepted the task. Contact the site administrator for rejecting the task."})

            # Update worker decision to Reject
            notification.worker_decision = 'Reject'
            notification.save()

            # Return success message
            return JsonResponse({"message": "Task has been rejected successfully by the worker"})
